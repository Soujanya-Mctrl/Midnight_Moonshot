...
const res = await fetch('https://indexer.preview.midnight.network/api/v4/graphql', {
  ...
  body: JSON.stringify({
    query: `
      query GetState($address: HexEncoded!) {
...
if (data.data?.contractAction?.state) {
  const stateHex = data.data.contractAction.state;
  const contractState = ContractState.deserialize(fromHex(stateHex));
  const ledger = feedbackLedger(contractState.data);
  console.log('SUCCESS! Deserialized Ledger:', {
    totalResponses: Number(ledger.totalResponses),
    ratingSum: Number(ledger.ratingSum),
    positiveCount: Number(ledger.positiveCount),
  });
} else {
  console.log('No state yet or indexer response:', data);
}
